...
parser.add_argument('--tokenizer', type=str, default='gpt2', help='path or name of pre-trained HF Tokenizer')
...
if __name__ == '__main__':
    ...
    # if accelerator.is_main_process and args.model_     is not None:
    ...
    if args.freeze_model_weights:
        for n, p in model.named_parameters():
            if 'memory' not in n and 'lora' not in n and 'adapter' not in n and 'aggr' not in n:
    ...
    if not args.validate_only:
        # train loop
        # logger.info(f'MODEL CHECKPOINT IS: {args.model_cpt}')
        # if args.model_cpt is not None:
        #     trainer.load(args.model_cpt, reset_iteration=args.reset_iteration)
        ...
        if args.save_best:
            ...
            trainer.load(best_model_path, args.reset_optimizer, args.reset_lr, args.reset_iteration)
